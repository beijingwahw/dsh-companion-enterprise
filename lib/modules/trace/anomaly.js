...
/** 树高上限 ceil(log2(ψ))。 */
function maxTreeHeight(sampleSize) {
...
export function extractFeatures(trace) {
...
/** 构建一棵孤立树（递归，高度受限）。 */
function buildTree(points, height, heightLimit, rng) {
...
function harmonicAdjustment(size) {
    if (size <= 1)
        return 0;
    let harmonic = 0;
...
/** 评分数组：s(x) = 2^(−E[h]/c(ψ))。 */
function anomalyScores(points, trees, subsampleSize) {
    const cPsi = harmonicAdjustment(subsampleSize) || 1;
    return points.map((point) => {
        let total = 0;
        for (const tree of trees)
            total += pathLength(point, tree, 0);
 ...
 */
export function detectTraceAnomalies(traces, limit = DEFAULT_LIMIT, seed = 0x1f0_e57) {
    ...
    return {
        ...
        note: '',
        summary: `${count} 条轨迹 × ${TREE_COUNT} 棵孤立树（ψ=${subsampleSize}），` +
