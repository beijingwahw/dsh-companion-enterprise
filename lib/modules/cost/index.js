...
import { buildForecastReport } from './forecast.js';
import { shapleyAllocate } from './shapley.js';
import { DEFAULT_SIMILARITY_THRESHOLD, SemanticCacheStore } from './semantic.js';
...
export function apply(ctx) {
    ...
    ctx.effect(() => ctx.companion.http.add('GET', '/cost/state', async (_req, res) => {
        ...
        sendJson(res, 200, {
            ...
            adaptiveRouting: settings.adaptiveRouting,
            budget,
    ...
    ctx.effect(() => ctx.companion.http.add('POST', '/cost/settings', async (_req, res, { body }) => {
        ...
        if (record.adaptiveRouting !== undefined) {
            patch.adaptiveRouting = requireBoolean(record.adaptiveRouting, 'adaptiveRouting');
        }
    ...
    // 自适应路由面板：两类任务各自的赌臂统计（均值奖励/UCB/失败率/平均成本）。
    ctx.effect(() => ctx.companion.http.add('GET', '/cost/adaptive', async (_req, res) => {
        const settings = scope.get();
        const gateway = ctx.get('companionCost');
        if (!gateway)
            throw new HttpError('成本网关尚未就绪', 503);
        sendJson(res, 200, {
            enabled: settings.modelRouting && settings.adaptiveRouting,
            arms: await gateway.adaptiveReport(),
        });
    }), 'companion.cost-http-adaptive');
    // 清空自适应路由学习状态（可选 cls=simple|complex 只清一类）。
    ctx.effect(() => ctx.companion.http.add('POST', '/cost/adaptive/reset', async (_req, res, { query }) => {
        const gateway = ctx.get('companionCost');
        if (!gateway)
            throw new HttpError('成本网关尚未就绪', 503);
        const raw = query.get('cls');
        if (raw !== null && raw !== 'simple' && raw !== 'complex') {
            throw new HttpError('cls 只能是 simple 或 complex', 400);
        }
        await gateway.adaptiveReset(raw ?? undefined);
        sendJson(res, 200, { ok: true });
    }), 'companion.cost-http-adaptive-reset');
    // 成本预测哨兵：Holt 指数平滑外推 + 预算耗尽 ETA + CUSUM 突变检测。
    // days 参数可指定拟合窗口（默认 28 天，含今日）。
    ctx.effect(() => ctx.companion.http.add('GET', '/cost/forecast', async (_req, res, { query }) => {
        const { usage } = await ctx.companion.ready;
        const now = Date.now();
        const today = beijingDayKey(now);
        const rawDays = Number(query.get('days') ?? 28);
        const windowDays = Number.isFinite(rawDays) && rawDays >= 7 && rawDays <= 120 ? Math.floor(rawDays) : 28;
        const from = beijingDayKey(now - (windowDays - 1) * 86_400_000);
        const rows = usage.range(from, today);
        const settings = scope.get();
        const monthTotal = usage.total(usage.month(now));
        sendJson(res, 200, buildForecastReport(rows, settings.dailyBudgetCny, settings.monthlyBudgetCny, monthTotal.costCny, today));
    }), 'companion.cost-http-forecast');
    // ------------------------------------------------------------------
    // 语义缓存（创新扩展）：MinHash-LSH 近重复请求复用（GPTCache 式降本）
    // ------------------------------------------------------------------
    /** 语义缓存单例（存储域就绪后创建一次；LSH 桶索引常驻内存）。 */
    let semanticCachePromise;
    function semanticCacheOf() {
        semanticCachePromise ??= ctx.companion.ready.then(({ domain }) => new SemanticCacheStore(domain));
        return semanticCachePromise;
    }
    // 近重复查找：命中直接复用历史响应并记账节省额。
    ctx.effect(() => ctx.companion.http.add('POST', '/cost/cache/lookup', async (_req, res, { body }) => {
        const record = readObject(body);
        ...
        const cache = await semanticCacheOf();
        const result = await cache.lookup(prompt, threshold);
        sendJson(res, 200, {
            hit: result.hit,
            similarity: result.similarity,
            savedTokens: result.savedTokens,
            savedCny: result.savedCny,
            entry: result.entry
                ? {
                    ...
                    lastHitAt: result.entry.lastHitAt,
                }
                : null,
        });
    }), 'companion.cost-http-cache-lookup');
    // 缓存写入：调用方拿到 miss 并真实执行调用后回填（prompt/response/
    // token 数与费用），供后续近重复请求复用。
    ctx.effect(() => ctx.companion.http.add('POST', '/cost/cache/store', async (_req, res, { body }) => {
        const record = readObject(body);
        const prompt = requireString(record.prompt, 'prompt');
        const response = requireString(record.response, 'response');
        const model = typeof record.model === 'string' && record.model.trim()
            ? record.model.trim()
            : 'deepseek-chat';
        const inputTokens = record.inputTokens === undefined ? undefined : requireBudgetCny(record.inputTokens, 'inputTokens');
        const outputTokens = record.outputTokens === undefined ? undefined : requireBudgetCny(record.outputTokens, 'outputTokens');
        const costCny = record.costCny === undefined ? undefined : requireBudgetCny(record.costCny, 'costCny');
        const cache = await semanticCacheOf();
    ...
    }), 'companion.cost-http-cache-store');
    // 缓存面板：容量/命中率/节省 token 与费用账本/最近条目。
    ctx.effect(() => ctx.companion.http.add('GET', '/cost/cache/stats', async (_req, res) => {
        const cache = await semanticCacheOf();
        sendJson(res, 200, cache.report());
    }), 'companion.cost-http-cache-stats');
    // 清空语义缓存（重置条目与统计）。
    ctx.effect(() => ctx.companion.http.add('DELETE', '/cost/cache', async (_req, res) => {
        const cache = await semanticCacheOf();
        await cache.clear();
        sendJson(res, 200, { ok: true });
    }), 'companion.cost-http-cache-clear');
    // ------------------------------------------------------------------
    // Shapley 成本公平分账（创新扩展）：阶梯折扣联合结余的边际贡献分配
    // ------------------------------------------------------------------
    // POST /cost/attribution {players, tiers}：各部门用量 + 厂商阶梯折扣表
    // → Shapley 值分账（≤8 玩家精确枚举，更多为种子化蒙特卡洛）。
    ctx.effect(() => ctx.companion.http.add('POST', '/cost/attribution', async (_req, res, { body }) => {
        const record = readObject(body);
        const rawPlayers = record.players;
        if (!Array.isArray(rawPlayers) || rawPlayers.length === 0) {
            throw new HttpError('players 必须是非空数组', 400);
        }
        if (rawPlayers.length > 24)
            throw new HttpError('players 不能超过 24 个', 400);
        const players = rawPlayers.map((raw, index) => {
            if (typeof raw !== 'object' || raw === null || Array.isArray(raw)) {
                throw new HttpError(`players[${index}] 必须是对象`, 400);
            }
            const entry = raw;
            const id = requireString(entry.id, `players[${index}].id`);
            const usage = requireBudgetCny(entry.usageCny, `players[${index}].usageCny`);
            return {
                id,
        ...
        if (!Array.isArray(rawTiers))
            throw new HttpError('tiers 必须是数组（可为空 = 无折扣）', 400);
        const tiers = rawTiers.map((raw, index) => {
            if (typeof raw !== 'object' || raw === null || Array.isArray(raw)) {
                throw new HttpError(`tiers[${index}] 必须是对象`, 400);
            }
            const entry = raw;
            const minCny = requireBudgetCny(entry.minCny, `tiers[${index}].minCny`);
            const discount = entry.discount;
            if (typeof discount !== 'number' || !Number.isFinite(discount) || discount < 0 || discount >= 1) {
                throw new HttpError(`tiers[${index}].discount 必须是 [0,1) 内的数字`, 400);
            }
            return { minCny, discount };
        });
        try {
            sendJson(res, 200, shapleyAllocate({ players, tiers }));
        }
        catch (error) {
            throw new HttpError(error instanceof Error ? error.message : '分账失败', 400);
        }
