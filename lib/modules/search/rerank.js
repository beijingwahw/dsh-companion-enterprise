...
export class ClickFeedbackStore {
    ...
    counter = 0;
    constructor(domain) {
        this.table = domain.table('search-clicks');
    }
    /** 记录一次展示（rerank 返回结果时调用）。 */
    async recordImpression(query, shown) {
        if (shown.length === 0)
            return;
        await this.put({
            kind: 'impression',
    ...
    /** 记录一次点击（位次从 1 起）。 */
    async recordClick(query, sessionId, position) {
        await this.put({
            kind: 'click',
            ts: Date.now(),
            query: normalizeQuery(query),
    ...
    async trim() {
        ...
        if (entries.length <= EVENT_KEEP_LIMIT)
            return;
        const stale = entries
...
function examinationPropensity(rank) {
    return 1 / Math.log2(rank + 2);
}
/** 累计一条事件到统计映射。 */
function bump(target, key, sessionId, patch) {
 ...
 */
export function learnClickModel(events) {
 ...
 * 均无证据返回 0（退化为纯引擎位次）。
 */
export function clickScore(model, query, sessionId) {
    ...
    for (const term of tokenizeQuery(normalized)) {
        ...
        if (!stats || stats.effectiveImpressions <= 0)
            continue;
...
function smoothedRate(effectiveClicks, effectiveImpressions, prior) {
    return Math.min(1, Math.max(0, (effectiveClicks + SMOOTHING_ALPHA * prior) / (effectiveImpressions + SMOOTHING_ALPHA)));
}
/** 保留 2 位小数。 */
 ...
 */
export function rerankHits(hits, model, query, weight = DEFAULT_CLICK_WEIGHT) {
...
/** 汇总模型面板（top 会话跨词元聚合）。 */
export function clickModelStats(model) {
