...
import { AUDIT_LOG_LIMIT, AuditAlertStore, AuditLogStore, DlpBlockStore, DlpRuleStore, DlpSettingsStore, NamedKeyStore, auditId, } from './store.js';
/** 插件名。 */
...
export function apply(ctx) {
    void (async () => {
            ...
            ctx.effect(() => {
                        ...
                        if (injection.enabled) {
                            ...
                            if (result.verdict !== 'clean') {
                                ...
                                void sentinel
                                    ...
                                    .catch(() => undefined);
                                if (sentinelShouldBlock(level, result.verdict) ||
                                    (injection.strict && result.verdict === 'malicious')) {
                                    throw new HttpError(`【注入拦截】检测到提示注入攻击（风险 ${result.risk}/100）：${names}` +
                                        (level !== 'normal' ? `（攻击源已升级为 ${level} 级防御）` : '（严格模式，已阻止发送）'), 403);
                                }
                                ctx.companion.notice('warning', `【注入警告】检测到疑似提示注入（风险 ${result.risk}/100，${result.verdict}）：${names}` +
                                    (level !== 'normal' ? `；攻击源风险等级 ${level}` : ''));
                            }
                        }
                    },
                    afterCall(params, result, error, costCny) {
                        const ts = Date.now();
                        const text = params.messages.map((m) => m.content).join('\n');
                        ...
                        // 异常告警：调用频率突增。
                        recentCalls.push(ts);
                        while (recentCalls.length > 0 && recentCalls[0] < ts - 60_000)
                            recentCalls.shift();
                ...
                const disposers = [
                    ...
                    ctx.companion.http.add('POST', '/security/keys/activate', async (_req, res, hctx) => {
                        ...
                        if (!secret)
                            throw new HttpError(`Key「${name}」不存在或已损坏`, 404);
                    ...
                    ctx.companion.http.add('POST', '/security/keys/leak-check', async (_req, res, hctx) => {
                        ...
                        if (mainKey && content.includes(mainKey))
                            leaked.push('（当前激活 Key）');
                        // 命名 Key 泄露检测。
                    ...
                    ctx.companion.http.add('POST', '/security/dlp/rules', async (_req, res, hctx) => {
                        ...
                        if (problem)
                            throw new HttpError(`正则表达式非法：${problem}`, 400);
                        // 内置规则只允许切换启用状态。
                        const builtin = dlpRules.list().find((rule) => rule.builtin && rule.name === name);
                        if (builtin)
                            throw new HttpError('内置规则不可修改，请新建自定义规则', 400);
                    ...
                    ctx.companion.http.add('POST', '/security/dlp/rules/toggle', async (_req, res, hctx) => {
                        ...
                        if (!rule)
                            throw new HttpError(`规则不存在：${id}`, 404);
                        if (rule.builtin) {
                    ...
                    ctx.companion.http.add('DELETE', '/security/dlp/rules', async (_req, res, hctx) => {
                        ...
                        if (!rule)
                            throw new HttpError(`规则不存在：${id}`, 404);
                        if (rule.builtin)
                            throw new HttpError('内置规则不可删除', 400);
                    ...
                    ctx.companion.http.add('POST', '/security/kanonymize', (_req, res, hctx) => {
                        ...
                        if (rawRecords.length > 5000)
                            throw new HttpError('records 不能超过 5000 条', 400);
                        const k = body.k;
        ...
        function buildReport(from, to) {
            ...
            const usageTable = domain.table('usage-daily');
            let totalCalls = 0;
            let totalCostCny = 0;
            let totalTokens = 0;
            const modelCalls = {};
...
function keyView(meta, configured) {
    return {
        name: meta.name,
        note: meta.note,
...
function parseScope(raw) {
    ...
    const models = Array.isArray(record.models)
        ? record.models.filter((m) => typeof m === 'string' && m.trim().length > 0).map((m) => m.trim())
        : [];
...
/** 解析可选时间戳参数。 */
function parseOptionalTs(value) {
    if (!value)
        return undefined;
...
function requireDayRange(query) {
...
function buildReportHtml(report) {
    ...
    const alertRows = report.alerts
        .map((alert) => `<tr><td>${new Date(alert.ts).toLocaleString('zh-CN', { hour12: false })}</td><td>${escapeHtml(alert.kind)}</td><td>${escapeHtml(alert.detail)}</td></tr>`)
        .join('\n');
    return `<!doctype html>
<html lang="zh-CN">
<head>
