...
export function pipelineToYaml(pipeline) {
    ...
    for (const step of pipeline.steps) {
        ...
        if (step.fallbackModel)
            lines.push(`    fallbackModel: ${step.fallbackModel}`);
...
export class PipelineEngine {
    ...
    async runLoop(pipeline, run, signal) {
        ...
        for (;;) {
            ...
            for (const step of pipeline.steps) {
                ...
                // 信号量：并发已满 → 剩余就绪步骤下轮再取。
                if (inflight + ready.length >= MAX_PARALLEL_STEPS)
                    continue;
                ready.push(step);
            }
            if (hasFailure) {
            ...
            if (blockedCount > 0 && !inflightSteps && inflight === 0 && ready.length === 0) {
                if (!this.blockedSince.has(run.id))
                    this.blockedSince.set(run.id, Date.now());
    ...
    async runStep(pipeline, run, step, signal) {
        ...
        const fallbackModel = step.fallbackModel && step.fallbackModel !== step.model ? step.fallbackModel : '';
        ...
        if (primaryAdmitted) {
            for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
                ...
                try {
                    ...
                    const baseMs = errorClass === 'rate-limit'
                        ? Math.max(step.retryIntervalMs * 2, 10_000)
                        : step.retryIntervalMs;
                    if (attempt < maxAttempts)
                        await sleep(backoffDelay(attempt, baseMs, 30_000));
                }
            }
        }
        ...
        if (fallbackModel && this.breaker.admit(fallbackModel)) {
            ...
            record.attempts += 1;
            try {
                const result = await withTimeout(this.call({ prompt, model: fallbackModel, timeoutMs, source: 'orchestrator' }), timeoutMs, `步骤「${step.name}」降级模型执行超时（${Math.round(timeoutMs / 1000)}s）`);
                ...
                record.tokens = result.tokens;
                await this.runs.put(run);
                this.ctx.companion.notice('info', `流水线「${pipeline.name}」步骤「${step.name}」已自愈降级至 ${fallbackModel}`);
                return;
            }
            catch (error) {
...
export class QueueWorker {
    ...
    nextQueued() {
        ...
        for (const task of this.tasks.list()) {
            ...
            if (!this.breaker.peek(task.model))
                continue;
            if (!best) {
                best = task;
                continue;
...
export class CronTicker {
    ...
    async tick() {
        ...
        for (const job of this.jobs.list()) {
            ...
            if (!this.breaker.peek(job.model))
                continue;
    ...
    async executeJob(job) {
        ...
        if (!this.breaker.admit(job.model))
            return;
        try {
            const result = await this.call({
                prompt: job.prompt,
