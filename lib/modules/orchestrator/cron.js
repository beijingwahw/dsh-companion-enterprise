...
/** 解析单个 Cron 字段；非法时抛错。 */
function parseField(text, range) {
...
function beijingToTs(year, month, day, hour, minute) {
    return Date.UTC(year, month - 1, day, hour, minute) - BEIJING_OFFSET_MS;
}
/** 集合中 ≥ from 的最小值；无则 undefined。 */
function nextAtLeast(values, from) {
    let best;
    for (const value of values) {
        if (value >= from && (best === undefined || value < best))
            best = value;
    }
    return best;
}
 ...
 */
export function nextCronFire(cron, fromMs) {
 ...
 */
export function naturalLanguageToCron(text) {
    ...
    if (match) {
        ...
        if (!n || n < 1 || n > 59)
            throw new Error('间隔分钟数必须在 1-59 之间');
        return `*/${n} * * * *`;
    }
    // 每隔 N 小时 / 每 N 小时
    ...
    if (match) {
        const n = parseCnNumber(match[1]);
        if (!n || n < 1 || n > 23)
            throw new Error('间隔小时数必须在 1-23 之间');
        return `0 */${n} * * *`;
    }
