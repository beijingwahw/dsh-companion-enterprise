...
/** 三角分布抽样（u∈[0,1)）。 */
function sampleTriangular(a, m, b, u) {
    if (b <= a)
        return a;
...
/** 三点估算（历史样本 → a/m/b）。 */
function threePoint(samples, timeoutMs) {
    if (samples.length >= 3) {
 ...
 */
export function simulatePipeline(pipeline, runs, options = {}) {
    const iterations = Math.min(MAX_ITERATIONS, Math.max(MIN_ITERATIONS, Math.floor(options.iterations ?? DEFAULT_ITERATIONS)));
    const parallelismRaw = options.parallelism;
    const parallelism = typeof parallelismRaw === 'number' && parallelismRaw > 0 ? Math.floor(parallelismRaw) : null;
    ...
    for (let iter = 0; iter < iterations; iter += 1) {
        ...
        for (const id of topo) {
            ...
            if (!est)
                continue;
            durations.set(id, Math.max(0, Math.round(sampleTriangular(est.a, est.m, est.b, Math.random()))));
        }
        ...
        let cursor = topo[0];
        for (const id of topo) {
            if ((finish.get(id) ?? 0) > (finish.get(cursor) ?? 0))
                cursor = id;
        }
        const path = new Set();
        ...
        for (const id of path)
            criticality.set(id, (criticality.get(id) ?? 0) + 1);
    }
    // 汇总分布。
    totals.sort((a, b) => a - b);
    const mean = totals.reduce((s, v) => s + v, 0) / Math.max(1, totals.length);
    const variance = totals.reduce((s, v) => s + (v - mean) * (v - mean), 0) / Math.max(1, totals.length - 1);
    const total = {
    ...
    const advice = stepRows.length === 0
        ? '流水线没有步骤可模拟'
...
function simulateBounded(topo, byId, dependents, durations, workers) {
    ...
    const ready = topo.filter((id) => (inDegree.get(id) ?? 0) === 0);
    const running = [];
    let clock = 0;
    let completed = 0;
    const done = new Set();
    while (completed < topo.length) {
        ...
        if (running.length === 0)
            break; // 无可执行任务（理论不可达，防御）。
        // 推进到最早完成的事件。
        let nextIndex = 0;
        for (let i = 1; i < running.length; i += 1) {
            if (running[i].endAt < running[nextIndex].endAt)
                nextIndex = i;
        }
