            ...
            ctx.effect(() => {
                ...
                const disposers = [
                    ...
                    ctx.companion.http.add('GET', '/orchestrator/pipelines/yaml', (_req, res, hctx) => {
                        ...
                        if (!pipeline)
                            throw new HttpError(`流水线不存在：${id}`, 404);
                    ...
                    ctx.companion.http.add('POST', '/orchestrator/runs', (_req, res, hctx) => {
                        ...
                        if (!pipeline)
                            throw new HttpError(`流水线不存在：${pipelineId}`, 404);
                    ...
                    ctx.companion.http.add('POST', '/orchestrator/runs/resume', (_req, res, hctx) => {
                        ...
                        if (!run)
                            throw new HttpError(`执行记录不存在：${runId}`, 404);
                        ...
                        if (!pipeline)
                            throw new HttpError('对应流水线已被删除', 404);
                    ...
                    ctx.companion.http.add('POST', '/orchestrator/runs/pause', (_req, res, hctx) => {
                        const body = readObject(hctx.body);
                        const runId = requireString(body.runId, 'runId');
                        if (!engine.requestPause(runId))
                            throw new HttpError('该执行不在运行中', 400);
                        sendJson(res, 200, { ok: true });
                    }),
                    ...
                    ctx.companion.http.add('GET', '/orchestrator/runs/get', (_req, res, hctx) => {
                        ...
                        if (!run)
                            throw new HttpError(`执行记录不存在：${id}`, 404);
                    ...
                    ctx.companion.http.add('POST', '/orchestrator/queue', async (_req, res, hctx) => {
                        ...
                        const failurePolicy = body.failurePolicy === 'retry' || body.failurePolicy === 'notify' ? body.failurePolicy : 'skip';
                    ...
                    ctx.companion.http.add('POST', '/orchestrator/queue/cancel', async (_req, res, hctx) => {
                        ...
                        if (!task)
                            throw new HttpError(`任务不存在：${id}`, 404);
                    ...
                    ctx.companion.http.add('POST', '/orchestrator/queue/pause', async (_req, res, hctx) => {
                        ...
                        if (task.status !== 'queued')
                            throw new HttpError('只有排队中的任务可以暂停', 400);
                        task.status = 'paused';
                        await tasks.put(task);
                        sendJson(res, 200, { ok: true });
                    }),
                    ctx.companion.http.add('POST', '/orchestrator/queue/resume', async (_req, res, hctx) => {
                        ...
                        if (task.status !== 'paused')
                            throw new HttpError('只有已暂停的任务可以恢复', 400);
                        task.status = 'queued';
                        await tasks.put(task);
                        sendJson(res, 200, { ok: true });
                    }),
                    ...
                    ctx.companion.http.add('DELETE', '/orchestrator/queue', async (_req, res, hctx) => {
                        ...
                        if (!task)
                            throw new HttpError(`任务不存在：${id}`, 404);
                        if (task.status === 'running')
                            throw new HttpError('运行中的任务请先取消', 400);
                    ...
                    ctx.companion.http.add('POST', '/orchestrator/jobs/toggle', async (_req, res, hctx) => {
                        ...
                        if (!job)
                            throw new HttpError(`定时任务不存在：${id}`, 404);
                    ...
                    ctx.companion.http.add('POST', '/orchestrator/dag', async (_req, res, hctx) => {
                        const body = typeof hctx.body === 'object' && hctx.body !== null && !Array.isArray(hctx.body)
                            ? hctx.body
                            : {};
                        const pipelineId = typeof body.pipelineId === 'string' ? body.pipelineId : '';
                        const pipeline = pipelines.get(pipelineId);
                        if (!pipeline)
                            throw new HttpError('流水线不存在', 404);
                    ...
                    ctx.companion.http.add('POST', '/orchestrator/monte', async (_req, res, hctx) => {
                        ...
                        if (!pipeline)
                            throw new HttpError('流水线不存在', 404);
                        const iterations = Number.isFinite(Number(body.iterations)) && Number(body.iterations) > 0
                            ? Number(body.iterations)
                            : undefined;
                        const parallelismRaw = Number(body.parallelism);
                        const parallelism = Number.isFinite(parallelismRaw) && parallelismRaw > 0
                            ? Math.floor(parallelismRaw)
                            : null;
                        sendJson(res, 200, simulatePipeline(pipeline, runs.forPipeline(pipelineId), {
                            iterations,
                            parallelism,
                        }));
                    }),
                    ...
                    ctx.companion.http.add('POST', '/orchestrator/cpm', async (_req, res, hctx) => {
                        ...
                        if (typeof body.durationOverrides === 'object' &&
                            body.durationOverrides !== null &&
                            !Array.isArray(body.durationOverrides)) {
                            durationOverrides = {};
                            for (const [stepId, value] of Object.entries(body.durationOverrides)) {
                        ...
                        sendJson(res, 200, analyzeCriticalPath(pipeline, runs.forPipeline(pipelineId), durationOverrides ? { durationOverrides } : {}));
                    }),
                    // --------------------------------------------------------------
                    // 命令面板
                    // --------------------------------------------------------------
...
function requireNextFire(parsed) {
    return nextCronFire(parsed, Date.now()) ?? 0;
}
/** 解析流水线步骤数组。 */
function parseSteps(raw) {
    if (!Array.isArray(raw))
        throw new HttpError('steps 必须是数组', 400);
    if (raw.length === 0)
        throw new HttpError('steps 不能为空', 400);
    ...
    return raw.map((item, index) => {
        ...
        if (!name)
            throw new HttpError(`steps[${index}].name 必须是非空字符串`, 400);
        ...
        return {
            ...
            dependsOn,
            fallbackModel: typeof record.fallbackModel === 'string' && record.fallbackModel.trim() ? record.fallbackModel.trim() : '',
        };
    });
}
/** 读取必填查询参数。 */
