 ...
 */
import { renderStructuredForInjection } from './structured.js';
// ---------------------------------------------------------------------------
// 参数与阈值
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
/** 就绪度门：评估一份结构化交接（纯函数）。 */
export function assessReadiness(handoff) {
    const gapLists = [];
    const dimension = (key, label, score, gaps) => {
        gapLists.push(gaps);
    ...
    const danglingPaths = collectMatches(activeText, PATH_PATTERN).filter((path) => !referenceText.includes(path.toLowerCase()));
    const danglingCommands = collectMatches(activeText, COMMAND_PATTERN).filter((command) => !referenceText.includes(command.toLowerCase()));
    const danglingIds = collectMatches(activeText, ID_PATTERN).filter((id) => !referenceText.includes(id.toLowerCase()));
    ...
    const droppedWithoutReason = handoff.dispositions.filter((d) => d.action === 'dropped' && !(d.reason ?? '').trim());
    ...
    const score = Math.round(dimensions.reduce((sum, dim) => sum + (dim.score * dim.weight) / 100, 0));
    const grade = score >= 85 ? 'A' : score >= 70 ? 'B' : score >= 50 ? 'C' : 'D';
    const gaps = gapLists
        .flat()
        .sort((a, b) => severityRank(b.severity) - severityRank(a.severity));
    const blocking = gaps.some((gap) => gap.severity === 'critical');
    const summary = `就绪度 ${score}/100（${grade}）：` +
        (blocking
