...
import { assembleStructuredHandoff, buildStructuredHandoffPrompt, LINEAGE_DEPTH_WARN_THRESHOLD, LINEAGE_MARKER_PATTERN, LineageStore, renderStructuredForInjection, } from './structured.js';
...
export function apply(ctx) {
     ...
     * 锚定继承守门 → 世系记录落库。
     */
    async function generateStructured(sessionId) {
        let snapshot;
        try {
            snapshot = await ctx.sessionQuery.readSession(sessionId);
        }
        ...
        let handoff;
        let autoRestoredCount;
        try {
            ;
            ({ handoff, autoRestoredCount } = assembleStructuredHandoff(content, parent, String(sessionId)));
        }
        catch (error) {
            throw new HttpError(`结构化交接解析失败：${error instanceof Error ? error.message : String(error)}`, 502);
        }
     ...
     */
    ctx.effect(() => ctx.companion.http.add('POST', '/handoff/structured', async (_req, res, { body }) => {
        const record = readObject(body);
        const sessionId = SessionId(requireString(record.sessionId, 'sessionId'));
     ...
     * 武装给下一个新对话（经世代门闩）。
     */
    ctx.effect(() => ctx.companion.http.add('POST', '/handoff/distill', async (_req, res, { body }) => {
        const record = readObject(body);
        const sessionId = SessionId(requireString(record.sessionId, 'sessionId'));
        let snapshot;
        try {
        ...
        if (turns.length === 0)
            throw new HttpError('会话中没有可蒸馏的对话内容', 400);
        const recentTurns = clampIntParam(record.recentTurns, 1, 40, DEFAULT_RECENT_TURNS);
        const charBudget = clampIntParam(record.charBudget, 1_000, 60_000, DEFAULT_CHAR_BUDGET);
    ...
    ctx.effect(() => ctx.companion.http.add('GET', '/handoff/lineage', async (_req, res) => {
        const stores = await storesReady;
     ...
     */
    ctx.effect(() => ctx.companion.http.add('GET', '/handoff/readiness', async (_req, res, hctx) => {
        const stores = await storesReady;
        ...
        const handoff = handoffId
            ...
            : stores.lineage.listSummaries()[0] !== undefined
                ...
                : undefined;
        if (!handoff) {
            throw new HttpError(handoffId ? `交接 ${handoffId} 不存在` : '尚无任何结构化交接可评估', 404);
        }
     ...
     */
    ctx.effect(() => ctx.companion.http.add('GET', '/handoff/acceptance', async (_req, res, hctx) => {
        const stores = await storesReady;
        const handoffId = hctx.query.get('handoffId')?.trim() ?? '';
        const handoff = handoffId
            ? stores.lineage.get(handoffId)
            : stores.lineage.listSummaries()[0] !== undefined
                ...
                : undefined;
        if (!handoff) {
            throw new HttpError(handoffId ? `交接 ${handoffId} 不存在` : '尚无任何结构化交接可出卷', 404);
        }
     ...
     */
    ctx.effect(() => ctx.companion.http.add('POST', '/handoff/acceptance/grade', async (_req, res, { body }) => {
        const record = readObject(body);
        const handoffId = requireString(record.handoffId, 'handoffId');
        const stores = await storesReady;
        const handoff = stores.lineage.get(handoffId);
        if (!handoff)
            throw new HttpError(`交接 ${handoffId} 不存在`, 404);
        const rawAnswers = record.answers;
        if (!Array.isArray(rawAnswers))
            throw new HttpError('answers 必须是数组', 400);
     ...
     */
    ctx.effect(() => ctx.companion.http.add('GET', '/handoff/lineage/trace', async (_req, res, hctx) => {
        const handoffId = hctx.query.get('handoffId')?.trim() ?? '';
        if (handoffId.length === 0)
            throw new HttpError('handoffId 必填', 400);
        const stores = await storesReady;
        ...
        handler: async (invocation) => {
            ...
            try {
                ...
                const lines = [
                    result.rendered,
                    '',
                    `—— 世系：${result.handoff.handoffId}（第 ${result.handoff.depth + 1} 代，父代 ${result.handoff.parentHandoffId ?? '无（初代）'}）；守门补回锚定 ${result.autoRestoredCount} 条`,
                ];
