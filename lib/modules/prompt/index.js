...
import { MAX_CANDIDATES, MAX_OPTIMIZE_CASES, optimizePrompt } from './optimize.js';
import { BanditStore, MAX_BANDIT_ARMS, MAX_BANDIT_CASES, MAX_PULL_ROUNDS, posteriorAnalysis, runBanditPulls, } from './bandit.js';
            ...
            ctx.effect(() => {
                const disposers = [
                    ...
                    ctx.companion.http.add('POST', '/prompt/optimize', async (_req, res, hctx) => {
                        ...
                        const model = typeof body.model === 'string' && body.model.trim() ? body.model.trim() : 'deepseek-chat';
                        const candidates = body.candidates === undefined ? 2 : requireInt(body.candidates, 'candidates');
                    ...
                    ctx.companion.http.add('POST', '/prompt/compile', (_req, res, hctx) => {
                        ...
                        if (budget <= 0)
                            throw new HttpError('budgetTokens 必须是正整数', 400);
                        if (budget > 1_000_000)
                            throw new HttpError('budgetTokens 过大（上限 100 万）', 400);
                    ...
                    ctx.companion.http.add('POST', '/prompt/bandit', async (_req, res, hctx) => {
                        ...
                        if (variants.length < 2)
                            throw new HttpError('variants 至少需要 2 个变体', 400);
                        ...
                        if (cases.length === 0)
                            throw new HttpError('cases 至少需要 1 条用例', 400);
                        const model = typeof body.model === 'string' && body.model.trim() ? body.model.trim() : 'deepseek-chat';
                    ...
                    ctx.companion.http.add('GET', '/prompt/bandit/get', (_req, res, hctx) => {
                        ...
                        if (!experiment)
                            throw new HttpError(`实验不存在：${id}`, 404);
                        sendJson(res, 200, { experiment, analysis: posteriorAnalysis(experiment.arms) });
                    }),
                    ...
                    ctx.companion.http.add('POST', '/prompt/bandit/pull', async (_req, res, hctx) => {
                        ...
                        try {
                            ...
                            throw new HttpError(error instanceof Error ? error.message : '采样执行失败', 404);
                        }
                    }),
                    // 删除实验。
                    ...
                    ctx.companion.http.add('POST', '/prompt/lint', (_req, res, hctx) => {
                        ...
                        const variables = Array.isArray(body.variables) && body.variables.every((v) => typeof v === 'string')
                            ? body.variables
                            : undefined;
                        const budgetTokensRaw = Number(body.budgetTokens);
                        const budgetTokens = Number.isFinite(budgetTokensRaw) && budgetTokensRaw > 0
                            ? Math.floor(budgetTokensRaw)
                            : undefined;
                        sendJson(res, 200, lintPrompt(text, {
                            ...(variables ? { variables } : {}),
...
function parseOptimizeCases(value) {
...
function parseBanditCases(value) {
    if (!Array.isArray(value) || value.length === 0) {
        throw new HttpError('cases 必须是非空数组', 400);
    }
