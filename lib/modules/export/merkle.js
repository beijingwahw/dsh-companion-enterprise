...
/** 根哈希（hex；空树返回空串）。 */
export function merkleRootOf(leafHashes) {
 ...
 */
export function verifyMerkleProof(leafHash, index, proof, root) {
...
export function leafHashOf(fileName, content) {
...
export class MerkleStore {
    table;
    constructor(domain) {
        this.table = domain.table('export-merkle');
    }
    async save(record) {
...
/** 生成条目的包含证明（从已存批次）。 */
export function buildInclusionProof(record, fileName) {
    ...
    return {
        ...
        verifyHint: '复算方法：leaf = SHA-256(fileName + "\\n" + SHA-256(content))；' +
 ...
 * 3. 证明：叶 + 兄弟路径复算等于根。
 */
export function verifyInclusion(record, fileName, content, proof) {
    const index = record.entries.findIndex((entry) => entry.fileName === fileName);
