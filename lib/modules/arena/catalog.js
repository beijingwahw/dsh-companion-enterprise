...
/** 将自定义模型记录转换为目录条目。 */
export function customModelToInfo(record) {
    return {
        id: record.id,
        label: record.label,
        provider: 'external',
 ...
 */
export const ARENA_ALLOWED_ORIGINS = [
...
export function isAllowedArenaOrigin(baseUrl) {
    let origin;
    try {
...
function deriveLatencyTier(modelId) {
 ...
 */
export function deriveModelsFromIds(modelIds, exclude) {
    const curatedIds = new Set(CURATED_MODELS.map((model) => model.id));
    const derived = [];
    const seen = new Set();
    for (const modelId of modelIds) {
        if (exclude.has(modelId) || curatedIds.has(modelId) || seen.has(modelId))
            continue;
        seen.add(modelId);
...
function deriveDomesticModels() {
    return deriveModelsFromIds(Object.keys(CATALOG_TABLE), new Set());
}
/** 完整内置目录：精选模型 + 全国产派生模型。 */
export const ARENA_MODEL_CATALOG = [
    ...CURATED_MODELS,
 ...
 */
export function recommendModels(models, request, costPerCall, now = Date.now(), peakPricingModels = new Set()) {
    const offPeak = !isPeakTime(now);
    const candidates = models.filter((model) => {
        if (request.latencyRequirement === 'fast' && model.latencyTier === 'slow')
            return false;
        if (request.budgetPerCallCny > 0) {
            const cost = costPerCall[model.id];
            if (cost !== undefined && cost > request.budgetPerCallCny)
                return false;
        }
        return true;
    });
    ...
    const scored = candidates.map((model) => {
        ...
        const latencyScore = request.latencyRequirement === 'any'
            ? 0.8
        ...
        const reason = buildReason(model, request, cost, offPeak, costPerCall, peakPricingModels.has(model.id));
        return { model, score, reason };
    });
    return scored.sort((a, b) => b.score - a.score);
}
/** 生成推荐理由文案（hasPeakPricing=该模型厂商是否公布峰谷分时价）。 */
function buildReason(model, request, cost, offPeak, costPerCall, hasPeakPricing) {
    const parts = [];
    const accuracy = model.accuracyPrior[request.taskType] ?? 0.85;
    parts.push(`该任务类型准确率先验 ${(accuracy * 100).toFixed(0)}%`);
    if (cost > 0)
        parts.push(`单次估算成本 ¥${cost.toFixed(4)}`);
    if (offPeak) {
        // 与最贵候选对比，突出空闲时段性价比。
        ...
        if (pricier.length > 0) {
            ...
            if (topCost > 0 && cost > 0) {
                parts.push(`当前为空闲时段，成本仅为 ${topId} 的 ${(cost / topCost).toFixed(2)} 倍`);
            }
        }
