...
async function rasterize(html) {
    ...
    try {
        ...
        clone.setAttribute('style', `${bodyStyle};width:${IMAGE_WIDTH}px;background:#ffffff;`);
 ...
 * @throws 运行环境无法光栅化时抛出（无 canvas / SVG 解析失败）。
 */
export async function exportLongPng(html, fileName) {
 ...
 * @throws 运行环境无法光栅化时抛出。
 */
export async function exportRasterPdf(html, fileName) {
    const canvas = await rasterize(html);
