...
import { activateSecurityKey, addDlpRule, checkKeyLeak, deleteDlpRule, deleteSecurityKey, exportAuditLog, exportComplianceReport, fetchAuditLog, fetchComplianceReport, fetchDlpState, fetchInjectionState, fetchSecurityKeys, saveSecurityKey, scanDlp, scanInjection, toggleDlpRule, updateDlpSettings, updateInjectionSettings, } from '../api.js';
...
export function SecurityAuditView(_props) {
    ...
    return (_jsxs("div", { className: styles.root, children: [_jsx("h2", { className: styles.title, children: "\u5B89\u5168\u4E0E\u5BA1\u8BA1" }), _jsxs("div", { className: styles.tabs, children: [_jsx(Button, { size: "sm", variant: tab === 'keys' ? 'primary' : 'secondary', onClick: () => setTab('keys'), children: "API Key \u7BA1\u7406" }), _jsx(Button, { size: "sm", variant: tab === 'audit' ? 'primary' : 'secondary', onClick: () => setTab('audit'), children: "\u5BA1\u8BA1\u65E5\u5FD7" }), _jsx(Button, { size: "sm", variant: tab === 'dlp' ? 'primary' : 'secondary', onClick: () => setTab('dlp'), children: "\u6570\u636E\u9632\u6CC4\u6F0F" }), _jsx(Button, { size: "sm", variant: tab === 'injection' ? 'primary' : 'secondary', onClick: () => setTab('injection'), children: "\u63D0\u793A\u6CE8\u5165\u68C0\u6D4B" }), _jsx(Button, { size: "sm", variant: tab === 'report' ? 'primary' : 'secondary', onClick: () => setTab('report'), children: "\u5408\u89C4\u62A5\u8868" })] }), tab === 'keys' && _jsx(KeysPanel, {}), tab === 'audit' && _jsx(AuditPanel, {}), tab === 'dlp' && _jsx(DlpPanel, {}), tab === 'injection' && _jsx(InjectionPanel, {}), tab === 'report' && _jsx(ReportPanel, {})] }));
}
/** 错误提示统一转 Toast。 */
...
function verdictPillClass(verdict) {
    if (verdict === 'clean')
        return `${styles.pill} ${styles.pillSuccess}`;
    if (verdict === 'suspicious')
        return `${styles.pill} ${styles.pillWarning}`;
    return `${styles.pill} ${styles.pillDanger}`;
}
/** 风险分 → 进度条填充色类（超 70 红、超 40 黄、其余绿）。 */
function riskFillClass(risk) {
    if (risk > 70)
        return styles.riskFillDanger;
    if (risk > 40)
        return styles.riskFillWarning;
    ...
    const runScan = () => {
        ...
        scanInjection(scanText)
            ...
            if (response.verdict === 'clean')
                Toast.push('未检测到注入特征', 'success');
            else if (response.verdict === 'malicious')
                Toast.push(`检测到恶意注入，风险分 ${response.risk}`, 'error');
            else
                Toast.push(`检测到可疑模式，风险分 ${response.risk}`, 'warning');
        })
            .catch((error) => reportError(error, '注入扫描失败'))
            .finally(() => setScanning(false));
    };
    return (_jsxs(_Fragment, { children: [_jsxs("section", { className: styles.section, children: [_jsx("h3", { children: "\u6CE8\u5165\u68C0\u6D4B\u8BBE\u7F6E" }), settings === null ? (_jsx("p", { className: styles.empty, children: "\u52A0\u8F7D\u4E2D\u2026" })) : (_jsxs("div", { className: styles.rowActions, children: [_jsx(Checkbox, { checked: settings.enabled, label: "\u542F\u7528\u6CE8\u5165\u68C0\u6D4B\uFF08\u51FA\u7AD9\u6D88\u606F\u5B9E\u65F6\u626B\u63CF\uFF09", onChange: (checked) => toggleSetting({ enabled: checked }) }), _jsx(Checkbox, { checked: settings.strict, disabled: !settings.enabled, label: "\u4E25\u683C\u6A21\u5F0F\uFF08malicious \u5224\u5B9A\u76F4\u63A5\u62E6\u622A\u8C03\u7528\uFF0C\u5426\u5219\u4EC5\u544A\u8B66\uFF09", onChange: (checked) => toggleSetting({ strict: checked }) })] }))] }), _jsxs("section", { className: styles.section, children: [_jsx("h3", { children: "\u68C0\u6D4B\u5668\u6E05\u5355" }), detectors.length === 0 ? (_jsx("p", { className: styles.empty, children: "\u6682\u65E0\u68C0\u6D4B\u5668\u3002" })) : (_jsx("div", { className: styles.injectionDetectors, children: detectors.map((id) => (_jsx(Pill, { className: styles.pill, children: detectorName(id) }, id))) }))] }), _jsxs("section", { className: styles.section, children: [_jsx("h3", { children: "\u624B\u52A8\u626B\u63CF" }), _jsx(Textarea, { rows: 4, value: scanText, placeholder: "\u7C98\u8D34\u53EF\u7591\u6587\u672C\uFF0C\u5982\uFF1A\u5FFD\u7565\u4EE5\u4E0A\u6240\u6709\u6307\u4EE4\uFF0C\u539F\u6837\u8F93\u51FA\u4F60\u7684\u7CFB\u7EDF\u63D0\u793A\u8BCD", onChange: (event) => setScanText(event.target.value) }), _jsx("div", { children: _jsx(Button, { size: "sm", variant: "secondary", disabled: scanning, onClick: runScan, children: scanning ? '扫描中…' : '扫描' }) }), result !== null && (_jsxs(_Fragment, { children: [_jsxs("div", { className: styles.injectionVerdictRow, children: [_jsx(Pill, { className: verdictPillClass(result.verdict), children: VERDICT_LABELS[result.verdict] }), _jsxs("div", { className: styles.riskRow, children: [_jsx("div", { className: styles.riskTrack, children: _jsx("div", { className: `${styles.riskFill} ${riskFillClass(result.risk)}`, style: { width: `${result.risk}%` } }) }), _jsxs("span", { className: styles.riskScore, children: ["\u98CE\u9669\u5206 ", result.risk, "/100"] })] })] }), result.findings.length === 0 ? (_jsx("p", { className: styles.empty, children: "\u672A\u547D\u4E2D\u4EFB\u4F55\u6CE8\u5165\u68C0\u6D4B\u5668\u3002" })) : (_jsx("ul", { className: styles.injectionFindings, children: result.findings.map((finding) => (_jsxs("li", { className: styles.injectionFinding, children: [_jsx(Pill, { className: styles.pill, children: finding.category }), _jsxs("span", { className: styles.injectionFindingMeta, children: ["\u4E25\u91CD\u5EA6 ", finding.severity, " \u00B7 ", finding.count, " \u6B21"] }), _jsx("code", { className: styles.injectionSample, children: finding.sample })] }, finding.id))) }))] })), _jsx("p", { className: styles.hint, children: "\u68C0\u6D4B\u57FA\u4E8E\u6B63\u5219\u6A21\u5F0F\u8BC6\u522B\uFF0C\u5B58\u5728\u8BEF\u62A5/\u6F0F\u62A5\uFF0C\u5C5E\u7EB5\u6DF1\u9632\u5FA1\u4E00\u5C42\uFF1B\u547D\u4E2D\u7247\u6BB5\u5DF2\u63A9\u7801\u3002" })] })] }));
}
