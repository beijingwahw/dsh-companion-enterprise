...
import { batchQueue, cancelPipelineRun, cancelQueueTask, deleteJob, deletePipeline, deletePipelineRun, deleteQueueTask, fetchCircuits, fetchJobRuns, fetchJobs, fetchPipelineRun, fetchPipelineRuns, fetchPipelineYaml, fetchPipelines, fetchQueue, pausePipelineRun, pauseQueueTask, parseSchedule, resumePipelineRun, resumeQueueTask, saveJob, savePipeline, startPipelineRun, submitQueueTask, toggleJob, } from '../api.js';
import styles from './TaskOrchestratorView.module.css';
/** 任务编排视图页。 */
export function TaskOrchestratorView(_props) {
    const [tab, setTab] = useState('pipelines');
    return (_jsxs("div", { className: styles.root, children: [_jsx("h2", { className: styles.title, children: "\u4EFB\u52A1\u7F16\u6392" }), _jsxs("div", { className: styles.tabs, children: [_jsx(Button, { size: "sm", variant: tab === 'pipelines' ? 'primary' : 'secondary', onClick: () => setTab('pipelines'), children: "\u6D41\u6C34\u7EBF" }), _jsx(Button, { size: "sm", variant: tab === 'queue' ? 'primary' : 'secondary', onClick: () => setTab('queue'), children: "\u6279\u91CF\u961F\u5217" }), _jsx(Button, { size: "sm", variant: tab === 'jobs' ? 'primary' : 'secondary', onClick: () => setTab('jobs'), children: "\u5B9A\u65F6\u8C03\u5EA6" }), _jsx(Button, { size: "sm", variant: tab === 'circuits' ? 'primary' : 'secondary', onClick: () => setTab('circuits'), children: "\u6A21\u578B\u65AD\u8DEF\u5668" })] }), tab === 'pipelines' && _jsx(PipelinePanel, {}), tab === 'queue' && _jsx(QueuePanel, {}), tab === 'jobs' && _jsx(JobPanel, {}), tab === 'circuits' && _jsx(CircuitPanel, {})] }));
}
/** 错误提示统一转 Toast。 */
...
function CircuitStatePill(props) {
    ...
    return _jsx("span", { className: cls, children: CIRCUIT_STATE_LABELS[props.state] ?? props.state });
}
...
function CircuitPanel() {
    ...
    return (_jsxs("section", { className: styles.section, children: [_jsxs("div", { className: styles.sectionHeader, children: [_jsx("h3", { children: "\u6A21\u578B\u65AD\u8DEF\u5668" }), _jsx(Button, { size: "sm", variant: "secondary", onClick: () => load(false), children: "\u5237\u65B0" })] }), circuits === null ? (_jsx(Spinner, { label: "\u52A0\u8F7D\u65AD\u8DEF\u5668\u72B6\u6001\u2026" })) : circuits.length === 0 ? (_jsx("p", { className: styles.empty, children: "\u6682\u65E0\u65AD\u8DEF\u5668\u8BB0\u5F55\uFF1A\u6A21\u578B\u8C03\u7528\u5931\u8D25\u7D2F\u79EF\u540E\u81EA\u52A8\u51FA\u73B0\u3002" })) : (_jsx("div", { className: styles.circuitList, children: circuits.map((row) => (_jsxs("div", { className: styles.circuitCard, children: [_jsxs("div", { className: styles.circuitHead, children: [_jsx("span", { className: styles.circuitModel, children: row.model }), _jsx(CircuitStatePill, { state: row.state })] }), _jsxs("div", { className: styles.circuitMeta, children: ["\u8FDE\u7EED\u5931\u8D25 ", row.failures, " \u6B21 \u00B7 \u7194\u65AD\u65F6\u95F4 ", row.state === 'closed' ? '-' : formatTime(row.openedAt), " \u00B7 \u63A2\u9488\u65F6\u95F4", ' ', formatTime(row.probeAt)] }), legend[row.state] ? _jsx("p", { className: styles.circuitDesc, children: legend[row.state] }) : null] }, row.model))) })), Object.keys(legend).length > 0 && (_jsx("div", { className: styles.circuitLegendBar, children: Object.entries(legend).map(([state, text]) => (_jsxs("span", { className: styles.circuitLegendItem, children: [CIRCUIT_STATE_LABELS[state] ?? state, "\uFF1A", text] }, state))) })), _jsx("p", { className: styles.hint, children: "\u6D41\u6C34\u7EBF/\u961F\u5217/\u5B9A\u65F6\u4EFB\u52A1\u5728 open \u671F\u95F4\u81EA\u52A8\u907F\u8BA9\u8BE5\u6A21\u578B\uFF1Bhalf-open \u653E\u884C\u5355\u4E2A\u63A2\u9488\u8C03\u7528\u9A8C\u8BC1\u6062\u590D\u3002" })] }));
}
