...
import { addArenaCustomModel, downloadBlob, fetchArenaModels, fetchArenaRecommendation, fetchCanaryOverview, fetchCanaryReport, removeArenaCustomModel, removeArenaKey, resetCanaryBaseline, runArenaCompare, runArenaLeaderboard, runCanaryProbes, saveArenaKey, } from '../api.js';
...
export function ModelArenaView(_props) {
    ...
    return (_jsxs("div", { className: styles.root, children: [_jsx("h2", { className: styles.title, children: "\u591A\u6A21\u578B\u7ADE\u6280\u573A" }), _jsxs("div", { className: styles.tabs, children: [_jsx(Button, { size: "sm", variant: tab === 'compare' ? 'primary' : 'secondary', onClick: () => setTab('compare'), children: "\u5E76\u884C\u5BF9\u6BD4" }), _jsx(Button, { size: "sm", variant: tab === 'leaderboard' ? 'primary' : 'secondary', onClick: () => setTab('leaderboard'), children: "\u8BC4\u6D4B\u6392\u884C\u699C" }), _jsx(Button, { size: "sm", variant: tab === 'recommend' ? 'primary' : 'secondary', onClick: () => setTab('recommend'), children: "\u6A21\u578B\u63A8\u8350" }), _jsx(Button, { size: "sm", variant: tab === 'drift' ? 'primary' : 'secondary', onClick: () => setTab('drift'), children: "\u6F02\u79FB\u76D1\u63A7" }), _jsx(Button, { size: "sm", variant: tab === 'keys' ? 'primary' : 'secondary', onClick: () => setTab('keys'), children: "\u6A21\u578B\u4E0E Key \u7BA1\u7406" })] }), tab === 'compare' && _jsx(ComparePanel, { models: models }), tab === 'leaderboard' && _jsx(LeaderboardPanel, { models: models }), tab === 'recommend' && _jsx(RecommendPanel, {}), tab === 'drift' && _jsx(DriftPanel, { models: models }), tab === 'keys' && _jsx(KeysPanel, { models: models, onChanged: reloadModels })] }));
}
/** 模型多选（最多 5 个）。 */
...
function DriftPanel(props) {
    ...
    const loadOverview = useCallback(() => {
        fetchCanaryOverview()
            ...
            .catch((err) => Toast.push(err instanceof Error ? err.message : '加载漂移概览失败', 'error'));
    }, []);
    useEffect(() => {
        loadOverview();
    }, [loadOverview]);
    ...
    /** 重置基线（确认厂商更新模型后重新锚定）。 */
    const resetBaseline = useCallback(async (model) => {
        if (!window.confirm(`确认重置 ${model} 的漂移基线？历史比对将清空，下次运行探针重新锚定。`))
            return;
        try {
    ...
    return (_jsxs("section", { className: styles.section, children: [_jsx("p", { className: styles.hint, children: "\u63A2\u9488\u4E3A\u786E\u5B9A\u6027\u8C03\u7528\uFF0C\u9996\u6B21\u8FD0\u884C\u5EFA\u7ACB\u57FA\u7EBF\uFF0C\u4E4B\u540E\u7D2F\u79EF\u5386\u53F2\u6BD4\u5BF9\uFF1B\u786E\u8BA4\u5382\u5546\u66F4\u65B0\u6A21\u578B\u540E\u53EF\u91CD\u7F6E\u57FA\u7EBF\u91CD\u65B0\u951A\u5B9A\u3002" }), _jsx("h3", { children: "\u6A21\u578B\u9009\u62E9\uFF08\u6700\u591A 5 \u4E2A\uFF09" }), _jsx("div", { className: styles.modelGrid, children: catalog.map((model) => (_jsx("label", { className: styles.modelOption, children: _jsx(Checkbox, { checked: selected.has(model.id), label: `${model.label}${model.provider === 'external' ? (model.keyConfigured ? '（Key 已配置）' : '（未配置 Key）') : ''}`, onChange: () => toggle(model.id) }) }, model.id))) }), _jsx("div", { className: styles.row, children: _jsx(Button, { variant: "primary", size: "sm", disabled: busy || selected.size === 0, onClick: () => void run(), children: busy ? '探针运行中…' : '运行探针并比对基线' }) }), busy && _jsx(Spinner, { label: "\u6B63\u5728\u8FD0\u884C\u786E\u5B9A\u6027\u63A2\u9488\u7EC4\u5E76\u6BD4\u5BF9\u57FA\u7EBF\u2026" }), runReports.length > 0 && (_jsxs(_Fragment, { children: [_jsx("h3", { children: "\u672C\u8F6E\u63A2\u9488\u62A5\u544A" }), runReports.map((report) => (_jsx(DriftReportDetail, { report: report }, report.model)))] })), _jsxs("h3", { children: ["\u53D7\u76D1\u63A7\u6A21\u578B\u6982\u89C8\uFF08", overview.length, "\uFF09"] }), overview.length === 0 ? (_jsx("p", { className: styles.hint, children: "\u5C1A\u65E0\u76D1\u63A7\u6570\u636E\uFF1A\u5148\u52FE\u9009\u6A21\u578B\u8FD0\u884C\u4E00\u6B21\u63A2\u9488\u5EFA\u7ACB\u57FA\u7EBF\u3002" })) : (_jsxs("table", { className: styles.table, children: [_jsx("thead", { children: _jsxs("tr", { children: [_jsx("th", { children: "\u6A21\u578B" }), _jsx("th", { children: "\u57FA\u7EBF\u951A\u70B9" }), _jsx("th", { children: "\u5386\u53F2\u8FD0\u884C" }), _jsx("th", { children: "\u5224\u5B9A" }), _jsx("th", { children: "\u64CD\u4F5C" })] }) }), _jsx("tbody", { children: overview.map((row) => (_jsxs("tr", { className: styles.driftRow, title: "\u70B9\u51FB\u67E5\u770B\u6F02\u79FB\u8BE6\u60C5", onClick: () => void openDetail(row.model), children: [_jsx("td", { children: row.model }), _jsx("td", { children: formatTimestamp(row.baselineTs) }), _jsxs("td", { children: [row.historyRuns, " \u6B21"] }), _jsx("td", { children: _jsx(DriftLevelPill, { level: row.verdict }) }), _jsx("td", { onClick: (event) => event.stopPropagation(), children: _jsxs("div", { className: styles.row, children: [_jsx(Button, { size: "sm", variant: "secondary", onClick: () => void openDetail(row.model), children: "\u67E5\u770B\u8BE6\u60C5" }), _jsx(Button, { size: "sm", variant: "danger", disabled: busy, onClick: () => void resetBaseline(row.model), children: "\u91CD\u7F6E\u57FA\u7EBF" })] }) })] }, row.model))) })] })), detail && (_jsxs(_Fragment, { children: [_jsxs("div", { className: styles.driftDetailHeader, children: [_jsxs("h3", { children: [detail.model, " \u00B7 \u6F02\u79FB\u8BE6\u60C5"] }), _jsx(Button, { size: "sm", variant: "ghost", onClick: () => setDetail(undefined), children: "\u6536\u8D77" })] }), _jsxs("p", { className: styles.driftMeta, children: ["\u57FA\u7EBF\u951A\u70B9 ", formatTimestamp(detail.baselineTs), " \u00B7 \u5386\u53F2\u8FD0\u884C ", detail.historyRuns, " \u6B21 \u00B7 \u63A2\u9488\u7EC4\uFF1A", detail.probes.join('、')] }), _jsx(DriftReportDetail, { report: detail.report })] }))] }));
}
/** 漂移级别 Pill（稳定=绿 / 预警=黄 / 漂移=红）。 */
function DriftLevelPill(props) {
    const meta = DRIFT_LEVEL_META[props.level];
    return _jsx(Pill, { className: meta.cls, children: meta.label });
}
/** 单份漂移报告详情：四维度统计表（统计量/阈值/判定/说明）+ 汇总结论。 */
function DriftReportDetail(props) {
    const { report } = props;
    return (_jsxs("div", { className: styles.driftDetail, children: [_jsxs("div", { className: styles.driftDetailHeader, children: [_jsx("strong", { children: report.model }), _jsx(DriftLevelPill, { level: report.verdict }), _jsxs("span", { className: styles.driftMeta, children: ["\u57FA\u7EBF\u951A\u70B9 ", formatTimestamp(report.baselineTs), " \u00B7 \u5DF2\u6BD4\u5BF9 ", report.runsCompared, " \u6B21"] })] }), _jsxs("table", { className: styles.table, children: [_jsx("thead", { children: _jsxs("tr", { children: [_jsx("th", { children: "\u7EF4\u5EA6" }), _jsx("th", { children: "\u7EDF\u8BA1\u91CF" }), _jsx("th", { children: "\u6F02\u79FB\u9608\u503C" }), _jsx("th", { children: "\u5224\u5B9A" }), _jsx("th", { children: "\u8BF4\u660E" })] }) }), _jsx("tbody", { children: report.dimensions.map((dimension) => (_jsxs("tr", { children: [_jsx("td", { children: DRIFT_DIMENSION_LABELS[dimension.name] }), _jsx("td", { children: dimension.statistic }), _jsx("td", { children: dimension.threshold }), _jsx("td", { children: _jsx(DriftLevelPill, { level: dimension.level }) }), _jsx("td", { children: dimension.detail })] }, dimension.name))) })] }), _jsx("p", { className: styles.driftSummary, children: report.summary })] }));
}
