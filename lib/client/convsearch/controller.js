 ...
 */
import { DEFAULT_MATCH_OPTIONS, clearPaint, findMatches, paint, resolveScope, scrollToMatch, } from './engine.js';
...
function countText(index, total) {
    return `${index} / ${total}`;
}
/**
...
class SearchController {
    ...
    /** 打开搜索栏（无对话渲染时无操作）。 */
    open() {
        if (this.bar === null || this.input === null)
            return;
    ...
    toggleOption(key) {
        ...
        if (btn !== null)
            btn.setAttribute('aria-pressed', String(this.state.options[key]));
        clearTimeout(this.searchTimer);
        this.runSearch(true);
        this.input?.focus();
    }
     ...
     */
    browseHistory(delta) {
        if (this.input === null || this.history.length === 0)
            return;
    ...
    relocateIndex(result) {
        ...
        if (anchor !== null) {
            ...
            // 同节点、偏移移动（周围文本增删）。
            for (let i = 0; i < result.total; i += 1) {
                if (result.matches[i]?.range.startContainer === anchor.container)
                    return i;
            }
        }
        // 无锚点或锚点消失：原地钳制。
    ...
    step(delta) {
        ...
        if (result.total === 0) {
            if (this.state.open && this.state.query.trim() !== '')
    ...
    watchScope() {
        const scope = resolveScope();
        if (scope === null || scope === this.observedScope)
            return;
        this.unwatchScope();
     ...
     */
    onKeyDown = (e) => {
        const isFind = (e.ctrlKey || e.metaKey) && !e.altKey && !e.shiftKey && (e.key === 'f' || e.key === 'F');
        if (isFind) {
        ...
        const isNav = e.key === 'F3' || ((e.ctrlKey || e.metaKey) && !e.altKey && (e.key === 'g' || e.key === 'G'));
        if (isNav) {
            e.preventDefault();
            e.stopPropagation();
            if (e.shiftKey)
