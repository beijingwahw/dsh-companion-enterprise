...
export class SessionIndex {
  ...
  async ensure(): Promise<void> {
    if (this.entries.size > 0 && Date.now() - this.builtAt < this.ttlMs) return
 ...
 */
export function pseudoRelevanceFeedback(
  index: SessionIndex,
  queryText: string,
  opts: { neighborhoodSize?: number; maxTerms?: number } = {},
): PrfResult {
  ...
  const scored = index
    ...
    .map((entry) => ({
      entry,
      similarity:
        0.7 * containment(queryShingles, entry.shingles) + 0.3 * jaccard(queryShingles, entry.shingles),
    }))
  ...
  for (const { entry } of scored) {
    for (const [term, tf] of entry.terms) {
      ...
      if (df > stopwordFloor) continue
      candidates.set(term, (candidates.get(term) ?? 0) + tf)
    }
  }
  const weightOf = (term: string, tf: number): number =>
    tf * Math.log(1 + total / Math.max(1, index.dfOf(term)))
  const pickTop = (minTf: number): ExpansionTerm[] =>
    [...candidates.entries()]
      .filter(([, tf]) => tf >= minTf)
 ...
 * 免去标定，是分布式检索的标准做法。
 */
export function reciprocalRankFuse(rankedLists: readonly (readonly string[])[], k = RRF_K): Map<string, number> {
...
export async function semanticSearch(
  deps: { sessionQuery: SessionQueryEngine; tagStore: TagStore },
  index: SessionIndex,
  query: string,
  limit: number,
  ...
  const matchedTermsOf = new Map<string, string[]>()
  ...
  try {
    ...
    for (const hit of page.hits) {
      if (hit.snippet) snippetOf.set(hit.session.id, hit.snippet)
    }
  } catch {
    // 引擎检索失败：退化为纯邻域排序。
  }
  ...
  for (const { term } of prf.expansionTerms) {
    try {
      ...
      rankedLists.push(page.hits.map((hit) => hit.session.id))
      for (const hit of page.hits) {
        if (hit.snippet && !snippetOf.has(hit.session.id)) snippetOf.set(hit.session.id, hit.snippet)
  ...
  const hits: SemanticHit[] = [...fused.entries()]
    .sort((a, b) => b[1] - a[1])
    .slice(0, limit)
    .map(([sessionId, score]) => {
      const entry = index.get(sessionId)
      return {
        session: entry?.record ?? ({ id: sessionId, createdAt: 0 } as SessionRecord),
 ...
 */
export async function similarSessions(
  deps: { sessionQuery: SessionQueryEngine; tagStore: TagStore },
  index: SessionIndex,
  sessionId: string,
  limit: number,
): Promise<SimilarSessionsResult> {
  await index.ensure()
  ...
  const total = Math.max(1, index.size)
  const hits: SimilarSessionHit[] = []
  for (const entry of index.list()) {
    if (entry.record.id === sessionId) continue
    const similarity =
      0.6 * jaccard(target.shingles, entry.shingles) + 0.4 * containment(target.shingles, entry.shingles)
