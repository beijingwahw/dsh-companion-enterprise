...
export interface EntityNeighborhood {
  ...
  /** 关联实体（边权降序，≤ 20 个）。 */
  readonly neighbors: ReadonlyArray<{ readonly name: string; readonly kind: EntityKind; readonly weight: number }>
  /** 关联会话（新→旧，≤ 20 个）。 */
...
const EXTRACTION_RULES: ReadonlyArray<{ readonly kind: EntityKind; readonly regex: RegExp }> = [
  // 文件路径：绝对/相对路径 + 常见扩展名。
  {
    kind: 'path',
...
/** 抽取一段文本中的实体（去重、规范化）。 */
function extractEntities(text: string): Map<string, EntityKind> {
  ...
  for (const rule of EXTRACTION_RULES) {
    ...
    for (const match of text.matchAll(rule.regex)) {
      ...
      if (rule.kind === 'acronym' && ACRONYM_STOPWORDS.has(raw.toUpperCase())) continue
      // 首个命中类别优先（path 优先于 acronym 等）。
      if (!found.has(raw)) found.set(raw, rule.kind)
      if (found.size >= ENTITIES_PER_SESSION_CAP) return found
    }
  }
  return found
}
 ...
 * 单会话读取失败跳过，不影响其余语料。
 */
export async function collectGraphSessions(
  sessionQuery: Pick<SessionQueryEngine, 'listSessions' | 'readSession'>,
  maxSessions: number = MAX_GRAPH_SESSIONS,
): Promise<Array<{ id: string; title: string | null; createdAt: number; text: string }>> {
  const sessions = [...(await sessionQuery.listSessions())]
    .sort((a, b) => b.createdAt - a.createdAt)
    .slice(0, maxSessions)
  const corpora: Array<{ id: string; title: string | null; createdAt: number; text: string }> = []
...
export function buildMemoryGraph(
  sessions: ReadonlyArray<{ id: string; title: string | null; createdAt: number; text: string }>,
): {
  entities: Map<string, { kind: EntityKind; sessionIds: Set<string> }>
  edges: Map<string, number>
  corpora: SessionCorpus[]
} {
  const entities = new Map<string, { kind: EntityKind; sessionIds: Set<string> }>()
  const edges = new Map<string, number>()
  const corpora: SessionCorpus[] = []
  for (const session of sessions) {
    const found = extractEntities(`${session.title ?? ''}\n${session.text}`)
    if (found.size === 0) continue
    for (const [name, kind] of found) {
      const entry = entities.get(name) ?? { kind, sessionIds: new Set<string>() }
...
function adjacency(
  ...
  for (const name of entities.keys()) adj.set(name, new Map())
...
/** PageRank 幂迭代。 */
function pageRank(adj: ReadonlyMap<string, ReadonlyMap<string, number>>): Map<string, number> {
  const nodes = [...adj.keys()]
  if (nodes.length === 0) return new Map()
  let rank = new Map<string, number>(nodes.map((n) => [n, 1 / nodes.length]))
...
export function graphReport(
  graph: ReturnType<typeof buildMemoryGraph>,
  hubLimit = 30,
): MemoryGraphReport {
  const adj = adjacency(graph.entities, graph.edges)
  const rank = pageRank(adj)
  const hubs: GraphEntity[] = [...graph.entities.entries()]
    .map(([name, entry]) => ({
      name,
      kind: entry.kind,
...
export function entityNeighborhood(
  ...
  const neighbors = [...(adj.get(name)?.entries() ?? [])]
    ...
    .map(([neighbor, weight]) => ({
      name: neighbor,
      kind: graph.entities.get(neighbor)?.kind ?? ('acronym' as EntityKind),
      weight,
    }))
