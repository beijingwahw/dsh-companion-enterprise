...
const HASH_FAMILY: readonly { readonly a: number; readonly b: number }[] = (() => {
  const rand = mulberry32(0x5eed_ca5c)
  const family: { a: number; b: number }[] = []
...
export function minHashSignature(text: string): number[] {
  ...
  if (grams.length === 0) return new Array<number>(SIGNATURE_SIZE).fill(0)
...
/** 单条带键（带内签名行拼接哈希）。 */
function bandKey(signature: readonly number[], band: number): string {
  let text = ''
...
/** 全部带键（LSH 桶索引的键集合）。 */
function bandKeys(signature: readonly number[]): string[] {
  const keys: string[] = []
...
export class SemanticCacheStore {
  ...
  constructor(
    domain: import('../../core/storage-adapter.js').Domain,
    private readonly now: () => number = Date.now,
  ) {
    this.table = domain.table<SemanticCacheRecord>('semantic-cache')
    for (const [key, record] of this.table.entries()) {
      if (record.kind === 'entry') this.indexAdd(key, record)
    }
  }

  private indexAdd(entryId: string, entry: SemanticCacheEntry): void {
  ...
  private indexRemove(entryId: string, signature: readonly number[]): void {
    for (const key of bandKeys(signature)) {
      this.buckets.get(key)?.delete(entryId)
    }
  }

  private statsRecord(): SemanticCacheStats {
    const record = this.table.get(STATS_KEY)
    if (record?.kind === 'stats') return record
    return {
      kind: 'stats',
      lookups: 0,
      hits: 0,
  ...
  private async bumpStats(patch: {
    lookups?: number
    hits?: number
    savedTokens?: number
    savedCny?: number
  }): Promise<SemanticCacheStats> {
    const prev = this.statsRecord()
    const next: SemanticCacheStats = {
      kind: 'stats',
  ...
  /** 条目是否仍在 TTL 内。 */
  private fresh(entry: SemanticCacheEntry, now: number): boolean {
  ...
  async lookup(
    ...
    for (const key of bandKeys(signature)) {
      for (const entryId of this.buckets.get(key) ?? []) candidates.add(entryId)
    }
    let best: SemanticCacheEntry | undefined
    let bestSimilarity = 0
    for (const entryId of candidates) {
      const record = this.table.get(entryId)
      if (record?.kind !== 'entry' || !this.fresh(record, now)) continue
    ...
    const updated: SemanticCacheEntry = {
      ...best,
      lastHitAt: now,
      hits: best.hits + 1,
    }
  ...
  async store(input: {
    ...
    for (const [key, record] of this.table.entries()) {
      ...
      if (normalizeText(record.prompt) === normalizeText(input.prompt)) {
        ...
        const replacement: SemanticCacheEntry = {
          kind: 'entry',
          entryId: key,
          prompt: input.prompt,
    ...
    if (entries.length >= CACHE_CAPACITY) {
      ...
      for (const entry of entries) {
        if (entry.lastHitAt < lru.lastHitAt) lru = entry
      }
  ...
  report(): SemanticCacheReport {
    ...
    const entries: SemanticCacheEntry[] = []
    for (const record of this.table.entries().map(([, v]) => v)) {
      if (record.kind === 'entry' && this.fresh(record, now)) entries.push(record)
    }
