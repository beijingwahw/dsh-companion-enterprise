...
export function apply(ctx: Context): void {
  ...
  ctx.effect(
    () =>
      ctx.companion.http.add('GET', '/cost/adaptive', async (_req, res) => {
        const settings = scope.get()
        const gateway = ctx.get('companionCost')
        if (!gateway) throw new HttpError('成本网关尚未就绪', 503)
        sendJson(res, 200, {
  ...
  ctx.effect(
    () =>
      ctx.companion.http.add('POST', '/cost/adaptive/reset', async (_req, res, { query }) => {
        const gateway = ctx.get('companionCost')
        if (!gateway) throw new HttpError('成本网关尚未就绪', 503)
  ...
  ctx.effect(
    () =>
      ctx.companion.http.add('GET', '/cost/forecast', async (_req, res, { query }) => {
        ...
        const windowDays =
          Number.isFinite(rawDays) && rawDays >= 7 && rawDays <= 120 ? Math.floor(rawDays) : 28
  ...
  let semanticCachePromise: Promise<SemanticCacheStore> | undefined

  function semanticCacheOf(): Promise<SemanticCacheStore> {
    semanticCachePromise ??= ctx.companion.ready.then(
      ({ domain }) => new SemanticCacheStore(domain),
    )
  ...
  ctx.effect(
    () =>
      ctx.companion.http.add('POST', '/cost/cache/lookup', async (_req, res, { body }) => {
        const record = readObject(body)
  ...
  ctx.effect(
    () =>
      ctx.companion.http.add('POST', '/cost/cache/store', async (_req, res, { body }) => {
        const record = readObject(body)
        const prompt = requireString(record.prompt, 'prompt')
        ...
        const model =
          typeof record.model === 'string' && record.model.trim()
            ...
            : 'deepseek-chat'
        const inputTokens =
          record.inputTokens === undefined ? undefined : requireBudgetCny(record.inputTokens, 'inputTokens')
        const outputTokens =
          record.outputTokens === undefined ? undefined : requireBudgetCny(record.outputTokens, 'outputTokens')
        const costCny =
          record.costCny === undefined ? undefined : requireBudgetCny(record.costCny, 'costCny')
        const cache = await semanticCacheOf()
  ...
  ctx.effect(
    () =>
      ctx.companion.http.add('GET', '/cost/cache/stats', async (_req, res) => {
        const cache = await semanticCacheOf()
  ...
  ctx.effect(
    () =>
      ctx.companion.http.add('DELETE', '/cost/cache', async (_req, res) => {
        const cache = await semanticCacheOf()
        await cache.clear()
        sendJson(res, 200, { ok: true })
      }),
  ...
  ctx.effect(
    () =>
      ctx.companion.http.add('POST', '/cost/attribution', async (_req, res, { body }) => {
        const record = readObject(body)
        const rawPlayers = record.players
        if (!Array.isArray(rawPlayers) || rawPlayers.length === 0) {
          throw new HttpError('players 必须是非空数组', 400)
        }
        if (rawPlayers.length > 24) throw new HttpError('players 不能超过 24 个', 400)
        ...
        if (!Array.isArray(rawTiers)) throw new HttpError('tiers 必须是数组（可为空 = 无折扣）', 400)
