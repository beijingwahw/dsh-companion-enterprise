...
function directiveOf(sentence: string): { polarity: 'positive' | 'negative'; marker: string; tail: string } | undefined {
  for (const marker of NEGATIVE_MARKERS) {
    const index = sentence.indexOf(marker)
    if (index >= 0) return { polarity: 'negative', marker, tail: sentence.slice(index + marker.length) }
  }
  for (const marker of POSITIVE_MARKERS) {
    const index = sentence.toLowerCase().indexOf(marker.toLowerCase())
    if (index >= 0) return { polarity: 'positive', marker, tail: sentence.slice(index + marker.length) }
  }
  return undefined
}
 ...
 */
export function lintPrompt(text: string, options: LintOptions = {}): PromptLintReport {
  ...
  // 指令句收集 + 极性分组。
  const directives: { sentence: string; polarity: 'positive' | 'negative'; tail: string }[] = []
  ...
  for (const positive of positives) {
    for (const negative of negatives) {
      if (positive.sentence === negative.sentence) continue
      const tailA = normalize(positive.tail)
      const tailB = normalize(negative.tail)
      if (tailA.length < 2 || tailB.length < 2) continue
      if (STOP_CHUNKS.has(positive.tail.trim()) || STOP_CHUNKS.has(negative.tail.trim())) continue
  ...
  const grade: PromptLintReport['grade'] = score >= 90 ? 'A' : score >= 75 ? 'B' : score >= 60 ? 'C' : 'D'
  const metrics: PromptMetrics = {
    ...
    directiveDensity:
      sentences.length > 0 ? Math.round((directives.length / sentences.length) * 100) / 100 : 0,
    hardConstraints,
    maxSentenceChars,
    nestingDepth,
  ...
  return {
    ...
    metrics,
    summary:
      `健康分 ${score}（${grade}）：${errors} 个错误、${warnings} 个警告、${infos} 个提示；` +
