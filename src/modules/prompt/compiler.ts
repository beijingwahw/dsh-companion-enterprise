...
/** 组件类别中文标签。 */
export const COMPONENT_KIND_LABELS: Readonly<Record<PromptComponentKind, string>> = {
  role: '角色设定',
  instruction: '任务指令',
  constraint: '约束条件',
  format: '输出格式',
...
const KIND_WEIGHT: Readonly<Record<PromptComponentKind, number>> = {
  role: 1.0,
  instruction: 0.95,
  constraint: 0.85,
  format: 0.8,
...
const DROP_ORDER: readonly PromptComponentKind[] = ['context', 'example', 'format', 'constraint']

/** 组件归类关键词（中文 + 英文常见形态）。 */
const KIND_PATTERNS: ReadonlyArray<{ readonly kind: PromptComponentKind; readonly regex: RegExp }> = [
...
function classifySegment(text: string): PromptComponentKind {
  const firstLine = text.trim().split('\n')[0].trim()
  for (const { kind, regex } of KIND_PATTERNS) {
    if (regex.test(firstLine)) return kind
  }
  return 'context'
}
...
function splitSegments(prompt: string): string[] {
  ...
  for (const segment of segments) {
    ...
    if (buffer.trim().length > 0) refined.push(buffer.trim())
  }
  return refined
}

/** 首尾滑窗压缩：保留开头 keepRatio 比例 + 结尾 1/4。 */
function windowCompress(text: string): string {
...
/** 示例截半：保留首个完整示例，丢弃其余。 */
function halveExample(text: string): string {
 ...
 */
export function compilePrompt(prompt: string, budgetTokens: number): CompileResult {
  const tokensBefore = estimateTokens(prompt)
  const components: PromptComponent[] = splitSegments(prompt).map((text) => ({
  ...
  // 工作副本：kind → 编译后文本。
  const work = new Map<number, { text: string; tokens: number; decision: ComponentDecision }>()
  components.forEach((c, i) => work.set(i, { text: c.text, tokens: c.tokens, decision: 'kept' }))
  let total = tokensBefore
  const report: CompiledComponent[] = []
  const record = (i: number): void => {
    const c = components[i]
    const w = work.get(i) as { text: string; tokens: number; decision: ComponentDecision }
    report.push({
      kind: c.kind,
      kindLabel: COMPONENT_KIND_LABELS[c.kind],
      before: c.text.slice(0, 80),
  ...
  for (const kind of DROP_ORDER) {
    for (let i = components.length - 1; i >= 0 && total > budgetTokens; i -= 1) {
      const c = components[i]
      const w = work.get(i) as { text: string; tokens: number; decision: ComponentDecision }
      if (c.kind !== kind || w.decision === 'dropped') continue
      total -= w.tokens
  ...
  components.forEach((_c, i) => {
    const w = work.get(i) as { text: string; tokens: number; decision: ComponentDecision }
    if (w.decision !== 'dropped' && w.text.length > 0) kept.push(w.text)
    record(i)
  })
