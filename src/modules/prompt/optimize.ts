...
async function generateCandidates(
  ...
  try {
    ...
    if (typeof parsed === 'object' && parsed !== null && 'candidates' in parsed) {
      ...
      const seen = new Set<string>([params.prompt])
      ...
      for (const item of list) {
        ...
        if (candidates.length >= params.candidates) break
      }
      return candidates
    }
    return []
 ...
 */
export function pairedSignTest(b: number, c: number): { pValue: number } {
  const n = b + c
...
export async function optimizePrompt(
  ...
  const candidateContents =
    baselineOk === params.cases.length ? [] : await generateCandidates(ctx, params, failures)
  ...
  for (const content of candidateContents) {
    ...
    for (let index = 0; index < params.cases.length; index += 1) {
      ...
      passes.push(pass)
      if (pass && !baselinePasses[index]) wins += 1
      if (!pass && baselinePasses[index]) losses += 1
    }
    candidates.push({
      content,
      passes,
  ...
  let significance: OptimizeResult['significance'] | undefined
  for (let index = 0; index < candidates.length; index += 1) {
    const candidate = candidates[index]
    if (candidate.wins <= candidate.losses) continue
  ...
  let savedVersion: PromptVersion | undefined
  if (winnerIndex !== undefined && significance?.significant && params.save) {
    const winner = candidates[winnerIndex]
    savedVersion = await versions.save(
      winner.content,
      `自动优化：基线 ${Math.round((baselineOk / params.cases.length) * 100)}% → ` +
        `${Math.round(winner.passRate * 100)}%（p=${significance.pValue}，配对胜 ${winner.wins} 负 ${winner.losses}）`,
      ['自动优化'],
    )
  }
