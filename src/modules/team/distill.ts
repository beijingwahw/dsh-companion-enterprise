 ...
 */
export function mineSignals(turns: readonly TranscriptTurn[]): MiningSignal[] {
  // 1. 收集全部（错误轮, 修复轮）候选对。
  interface Candidate {
    errorIndex: number
  ...
  for (let i = 0; i < turns.length; i += 1) {
    if (!ERROR_WORD.test(turns[i].text)) continue
    ...
    for (let j = i + 1; j <= Math.min(i + RECOVERY_WINDOW, turns.length - 1); j += 1) {
      ...
      if (!strongRecovery && stillError) continue
      ...
      const score = Math.min(
        0.35 * severity + 0.25 * distance + 0.25 * explicitness + 0.15 * userConfirm,
        1,
      )
  ...
  const signals: MiningSignal[] = []
  for (const candidate of candidates) {
    if (usedTurns.has(candidate.errorIndex) || usedTurns.has(candidate.recoveryIndex)) continue
    usedTurns.add(candidate.errorIndex)
    usedTurns.add(candidate.recoveryIndex)
    const context: TranscriptTurn[] = []
...
export function buildDistillPrompt(signal: MiningSignal): string {
 ...
 */
export function parseDistilledCard(raw: string): DistilledContent {
  ...
  const text = (key: string): string => {
    ...
    if (typeof value !== 'string' || !value.trim()) throw new Error(`模型输出缺少合法的 ${key}`)
    return value.trim()
  }
...
/** 卡片全文（去重比对文本）。 */
function cardTextOf(parts: { title: string; lesson: string; problem: string; solution: string; tags: readonly string[] }): string {
 ...
 * 最强证据），信号强度为辅。0-1。
 */
export function confidenceOf(card: DistilledCard): number {
...
export class DistilledCardStore {
  ...
  constructor(domain: Domain) {
    this.table = domain.table<DistilledCard>('distilled-cards')
  }
  ...
  async dedupPut(
    ...
    signal: MiningSignal,
    sessionId: string,
  ): Promise<{ card: DistilledCard; merged: boolean }> {
    ...
    const card: DistilledCard = {
      ...
      ] as const satisfies readonly EvidenceEntry[]).slice(0, EVIDENCE_CAP),
      promoted: false,
    }
  ...
  /** 标记已晋升。 */
  async markPromoted(id: string): Promise<DistilledCard | undefined> {
    let updated: DistilledCard | undefined
