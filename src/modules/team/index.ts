...
import type {
  ...
  SectionReport,
  SharedDlpRule,
  SharedHandoffTemplate,
  SharedJobRecord,
  SharedPipelineRecord,
  SharedPromptTemplate,
  TeamConfigSnapshot,
  TeamPreferences,
} from './types.js'
import {
  EXPERIENCE_CARD_LIMIT,
  ExperienceCardStore,
  ReviewCommentStore,
  ReviewDecisionStore,
  ReviewRequestStore,
...
export function apply(ctx: Context): void {
  void (async () => {
    ...
    /** 读取评审请求，不存在即 404。 */
    const requireReview = (id: string): ReviewRequest => {
      ...
      ctx.effect(() => {
        const disposers: Array<() => void> = [
          ...
          ctx.companion.http.add('POST', '/team/config/import', async (_req, res, hctx) => {
            ...
            const reports = await importSnapshot(domain, snapshot, strategy, (table) =>
              ctx.companion.setPricingOverrides(table),
            )
            if (strategy !== 'manual') {
          ...
          ctx.companion.http.add('DELETE', '/team/snapshots', async (_req, res, hctx) => {
            ...
            const table = domain.table<TeamConfigSnapshot>('team-snapshots')
            if (/^\d+$/.test(key)) {
          ...
          ctx.companion.http.add('POST', '/team/experience/notes', async (_req, res, hctx) => {
            ...
            if (!card) throw new HttpError(`执行卡片不存在：${id}`, 404)
            const updated: ExperienceCard = {
              ...card,
              notes: [
                ...card.notes,
                {
          ...
          ctx.companion.http.add('POST', '/team/reviews/decide', async (_req, res, hctx) => {
            ...
            const status: ReviewStatus = verdict === 'approve' ? 'approved' : 'rejected'
          ...
          ctx.companion.http.add('POST', '/team/reviews/merge', async (_req, res, hctx) => {
            ...
            if (review.status !== 'approved') throw new HttpError('仅通过审核的评审可合并主版本', 400)
            // 直接操作 'prompt-versions' 表：提议内容成为新的主版本。
            const versionsTable = domain.table<PromptVersionRecord>('prompt-versions')
            let latest = 0
            for (const [, record] of versionsTable.entries()) {
              if (Number.isInteger(record.version) && record.version > latest) latest = record.version
            }
          ...
          ctx.companion.http.add('DELETE', '/team/reviews', async (_req, res, hctx) => {
            ...
            // 级联清理评论与决定，避免留下孤儿记录。
            for (const comment of comments.forReview(id)) await comments.delete(comment.id)
            const decisionTable = domain.table<ReviewDecision>('review-decisions')
...
// --------------------------------------------------------------------
// 跨模块表记录形状（结构兼容，仅经 domain.table 直接读写）
// --------------------------------------------------------------------

/** 'templates' 表记录形状（与交接摘要模块结构兼容）。 */
interface TemplateRecord {
  content: string
  updatedAt: number
}

/** 'prompt-templates' 表记录形状（与 Prompt 工作台模块结构兼容）。 */
...
/** 'dlp-rules' 表记录形状（与安全模块结构兼容）。 */
interface DlpRuleRecord {
  id: string
  name: string
  pattern: string
  builtin: boolean
...
/** 'prompt-versions' 表记录形状（与 Prompt 工作台模块结构兼容）。 */
...
/** 列表型分区绑定：本地读取、远程读取、身份键与写入（收集/diff/导入共用）。 */
interface SectionBinding {
  readonly section: ConfigSection
  readonly keyOf: (item: unknown) => string
  readonly readLocal: () => readonly unknown[]
  readonly readRemote: (sections: TeamConfigSnapshot['sections']) => readonly unknown[] | undefined
...
function defineBinding<T>(
  ...
  readRemote: (sections: TeamConfigSnapshot['sections']) => readonly T[] | undefined,
  ...
  return {
    ...
    readLocal,
    readRemote,
    writeItem: (item) => writeItem(item as T),
  }
}

/** 构造五个列表型分区的绑定（本地读取统一投影为快照内形状，保证 diff 两侧可比）。 */
function buildSectionBindings(domain: Domain): SectionBinding[] {
  const templates = domain.table<TemplateRecord>('templates')
  const promptTemplates = domain.table<PromptTemplateRecord>('prompt-templates')
  const pipelines = domain.table<SharedPipelineRecord>('pipelines')
  const jobs = domain.table<SharedJobRecord>('scheduled-jobs')
  const dlpRules = domain.table<DlpRuleRecord>('dlp-rules')
  return [
    defineBinding<SharedHandoffTemplate>('handoffTemplates', (item) => item.name,
      () => templates.entries().map(([name, r]): SharedHandoffTemplate => ({ name, content: r.content })),
      (sections) => sections.handoffTemplates,
      (item) => templates.put(item.name, { content: item.content, updatedAt: Date.now() })),
    defineBinding<SharedPromptTemplate>('promptTemplates', (item) => item.name,
      () => promptTemplates.entries().map(([, r]) => r).filter((r) => !r.builtin)
        .map((r): SharedPromptTemplate => ({ name: r.name, category: r.category, content: r.content })),
      (sections) => sections.promptTemplates,
      (item) => promptTemplates.put(item.name, { ...item, builtin: false, updatedAt: Date.now() })),
    defineBinding<SharedPipelineRecord>('pipelines', (item) => item.id,
      () => pipelines.entries().map(([, record]) => record),
      (sections) => sections.pipelines,
      (item) => pipelines.put(item.id, item)),
    defineBinding<SharedJobRecord>('scheduledJobs', (item) => item.id,
      () => jobs.entries().map(([, record]) => record),
      (sections) => sections.scheduledJobs,
      (item) => jobs.put(item.id, item)),
    defineBinding<SharedDlpRule>('dlpRules', (item) => item.id,
      () => dlpRules.entries().map(([, r]) => r).filter((r) => !r.builtin)
        .map((r): SharedDlpRule => ({ id: r.id, name: r.name, pattern: r.pattern, enabled: r.enabled })),
      (sections) => sections.dlpRules,
      (item) => dlpRules.put(item.id, { ...item, builtin: false })),
  ]
}

/** 收集本地各表配置，组装导出快照（I1）。 */
function collectSnapshot(domain: Domain, exportedBy: string): TeamConfigSnapshot {
  const locals = new Map<ConfigSection, readonly unknown[]>()
  for (const binding of buildSectionBindings(domain)) locals.set(binding.section, binding.readLocal())
  const byName = (a: { name: string }, b: { name: string }): number => a.name.localeCompare(b.name, 'zh-CN')
  ...
  return {
    ...
    sections: {
      ...
      handoffTemplates: [...(locals.get('handoffTemplates') as readonly SharedHandoffTemplate[])].sort(byName),
      promptTemplates: [...(locals.get('promptTemplates') as readonly SharedPromptTemplate[])].sort(byName),
      pipelines: locals.get('pipelines') as readonly SharedPipelineRecord[],
      scheduledJobs: locals.get('scheduledJobs') as readonly SharedJobRecord[],
      dlpRules: locals.get('dlpRules') as readonly SharedDlpRule[],
    },
  }
}

/** 校验并规范化团队配置快照（diff 与导入共用入口）。 */
function parseSnapshot(raw: unknown): TeamConfigSnapshot {
...
function listSectionDiffs(
  domain: Domain,
  snapshot: TeamConfigSnapshot,
): Array<{ binding: SectionBinding; diffs: ConfigDiffEntry[] }> {
  return buildSectionBindings(domain).map((binding) => ({
    binding,
    diffs: diffPairs(
      binding.section,
      keyedPairs(binding.readLocal(), binding.keyOf),
      keyedPairs(binding.readRemote(snapshot.sections) ?? [], binding.keyOf),
    ),
  }))
}

/** 计算远程快照与本地配置的差异（costSettings 不产生 diff）。 */
function computeDiffs(domain: Domain, snapshot: TeamConfigSnapshot): ConfigDiffEntry[] {
...
async function importSnapshot(
  ...
  for (const { binding, diffs } of listSectionDiffs(domain, snapshot)) {
    if (binding.readRemote(snapshot.sections) === undefined) continue
  ...
  if (snapshot.sections.pricingOverrides !== undefined) {
    ...
    const report = await applyDiffs('pricingOverrides', diffs, strategy, async (entry) => {
      merged[entry.key] = entry.remote as ModelPrice
    })
    if (strategy !== 'manual' && (report.added > 0 || report.updated > 0)) {
      await domain.table<PriceTable>('cost-extra').put(COST_EXTRA_PRICING_KEY, merged)
...
function keyedPairs(items: readonly unknown[], keyOf: (item: unknown) => string): Array<[string, unknown]> {
  return items.map((item): [string, unknown] => [keyOf(item), item])
}
...
function diffPairs(
  section: ConfigSection,
  localPairs: ReadonlyArray<readonly [string, unknown]>,
  remotePairs: ReadonlyArray<readonly [string, unknown]>,
): ConfigDiffEntry[] {
  const entries: ConfigDiffEntry[] = []
  const localMap = new Map<string, unknown>(localPairs.map(([key, value]) => [key, value] as [string, unknown]))
  ...
  for (const [key, localItem] of localMap) {
    if (!seen.has(key)) entries.push({ section, key, action: 'local-only', local: localItem })
  }
  return entries
}
...
async function applyDiffs(
  ...
  strategy: MergeStrategy,
  write: (entry: ConfigDiffEntry) => Promise<void>,
): Promise<SectionReport> {
  let added = 0
  let updated = 0
  let same = 0
  ...
  for (const entry of diffs) {
    if (entry.action === 'same') {
      ...
      if (entry.action === 'update') updated += 1
      else added += 1
      if (strategy !== 'manual') await write(entry)
    }
  }
...
function readLocalPricing(domain: Domain): PriceTable {
  return domain.table<PriceTable>('cost-extra').get(COST_EXTRA_PRICING_KEY) ?? {}
}
...
function parseSectionArray<T>(
  ...
  if (!Array.isArray(raw)) throw new HttpError(`${field} 必须是数组`, 400)
  return raw.map((item, index) => parseItem(item, `${field}[${index}]`))
}

/** 解析交接摘要模板条目。 */
function parseHandoffTemplate(raw: unknown, field: string): SharedHandoffTemplate {
...
/** 解析 Prompt 模板条目。 */
function parsePromptTemplate(raw: unknown, field: string): SharedPromptTemplate {
  const record = requireObject(raw, field)
  return {
    name: requireString(record.name, `${field}.name`),
...
/** 解析流水线记录。 */
function parsePipeline(raw: unknown, field: string): SharedPipelineRecord {
  const record = requireObject(raw, field)
  return {
    id: requireString(record.id, `${field}.id`),
    name: requireString(record.name, `${field}.name`),
    steps: Array.isArray(record.steps) ? (record.steps as readonly unknown[]) : [],
...
/** 解析定时任务记录。 */
function parseScheduledJob(raw: unknown, field: string): SharedJobRecord {
  const record = requireObject(raw, field)
  ...
  return {
    ...
    cron,
    scheduleText:
      typeof record.scheduleText === 'string' && record.scheduleText.trim() ? record.scheduleText.trim() : cron,
...
/** 解析自定义 DLP 规则条目。 */
function parseDlpRule(raw: unknown, field: string): SharedDlpRule {
  const record = requireObject(raw, field)
  return {
    id: requireString(record.id, `${field}.id`),
    name: requireString(record.name, `${field}.name`),
...
/** 解析单价覆盖表（模型 id → 单价）。 */
function parsePriceTable(raw: Readonly<Record<string, unknown>> | undefined, field: string): PriceTable {
  if (raw === undefined) return {}
  const table: PriceTable = {}
  for (const [model, value] of Object.entries(raw)) table[model] = parseModelPrice(value, `${field}.${model}`)
  return table
}

/** 解析单模型单价（元/百万 tokens；inputCacheHit 缺省 0）。 */
function parseModelPrice(raw: unknown, field: string): ModelPrice {
  const record = requireObject(raw, field)
...
function readObject(body: unknown): Record<string, unknown> {
  if (!isPlainObject(body)) throw new HttpError('请求体必须是 JSON 对象', 400)
  return body
}

/** 读取必填非空字符串字段（自动去除首尾空白）。 */
function requireString(value: unknown, field: string): string {
  if (typeof value !== 'string' || !value.trim()) throw new HttpError(`${field} 必须是非空字符串`, 400)
  return value.trim()
}
...
function requireOptionalString(value: unknown, field: string): string | undefined {
  ...
  if (typeof value !== 'string') throw new HttpError(`${field} 必须是字符串`, 400)
  return value.trim()
}

/** 将值收窄为普通对象，否则 400。 */
function requireObject(value: unknown, field: string): Record<string, unknown> {
  if (!isPlainObject(value)) throw new HttpError(`${field} 必须是对象`, 400)
  return value
}
...
function parseStrategy(value: unknown, field: string): MergeStrategy {
  if (value === 'local' || value === 'remote' || value === 'manual') return value
...
function parseAnchor(value: unknown): ReviewAnchor {
  if (value === undefined || value === null) return { side: 'proposed', line: 0 }
  ...
  if (!Number.isInteger(line) || line < 0) throw new HttpError('anchor.line 必须是非负整数', 400)
