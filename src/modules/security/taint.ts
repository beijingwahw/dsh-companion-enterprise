...
/** 外发信道工具名特征。 */
const OUTBOUND_TOOL_RE =
  /http|fetch|request|webhook|curl|wget|post|send|mail|smtp|upload|submit|push|notify|browser|nav|share/i
...
function trackingKey(value: string): string {
...
function eventViews(snapshot: SessionLogSnapshot): EventView[] {
  const views: EventView[] = []
  for (const event of snapshot.events) {
    const data = event.data ?? {}
    const record = (data && typeof data === 'object' ? data : {}) as Record<string, unknown>
    let view: EventView | undefined
    switch (event.type) {
      case 'user/message': {
...
/** 工具调用信道分级。 */
function toolChannel(toolName: string): SinkChannel {
  if (OUTBOUND_TOOL_RE.test(toolName)) return 'outbound'
  if (STORAGE_TOOL_RE.test(toolName)) return 'storage'
  return 'internal'
}

function severityOf(channel: SinkChannel): 'high' | 'medium' | 'low' {
  if (channel === 'outbound') return 'high'
  if (channel === 'storage') return 'medium'
  return 'low'
}
...
export function trackTaint(
  ...
  const sourceKeys = new Map<string, TaintSource>()
  for (const view of views) {
    if (view.kind !== 'user') continue
    for (const match of findRawMatches(view.text, rules)) {
      const key = trackingKey(match.value)
      if (key.length < 4 || sourceKeys.has(key) || sourceKeys.size >= MAX_SOURCES) continue
      sourceKeys.set(key, {
        ruleId: match.ruleId,
  ...
  const taintedEvents = new Set<number>()
  const flows: TaintFlow[] = []
  for (const [key, source] of sourceKeys) {
    const hops: TaintHop[] = []
    let sink: SinkChannel = 'internal'
    let sinkLabel = '仅会话内传播'
    let truncated = false
    for (const view of views) {
      if (view.seq <= source.seq) continue
      if (!normalizeForSearch(view.text).includes(key)) continue
      ...
      const channel: SinkChannel = view.isModelCall
        ? 'model'
        : view.kind === 'tool-call'
  ...
  const riskLevel: TaintReport['riskLevel'] =
    outboundFlows > 0
      ? 'high'
      : storageFlows > 0
        ? 'medium'
